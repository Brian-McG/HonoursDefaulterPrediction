...
\begin{document}
	...
	We would like to investigate each data balancing technique in more detail so we compare each data balancing technique on each classifier. We use our parameter grid results from section \ref{parameter_tuning} to obtain the optimal parameters for each data balancing algorithm on each classifier. As before, the highest BARR is used to determine these optimal parameters. To ensure repeatable results, 10 runs of stratified 5-fold cross validation is executed and the results averaged. A set of unique randomly selected initialisation keys used for each run and recorded with the results. These are used so that $k$-fold cross validation, data balancing and classification algorithms produce deterministic output.
