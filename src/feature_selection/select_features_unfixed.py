    ...
    for data_set in data_sets.data_set_arr:
        if data_set["status"]:
            ...
            for feature_selection_strategy in feature_selection_strategies:
                input_set = input_defaulter_set_with_dummy_variables.iloc[:, :-1].as_matrix()
                output_set = input_defaulter_set_with_dummy_variables.iloc[:, -1:].as_matrix().flatten()
                grid_scores = None
                estimator = None
                if LOGISTIC_REGRESSION == feature_selection_strategy:
                    estimator = LogisticRegression(random_state=random_values[0], **data_set["data_set_classifier_parameters"].classifier_parameters["Logistic regression"]["classifier_parameters"])
                    ...
                    estimator = svm.SVC(random_state=random_values[0], kernel="linear", class_weight="balanced")

                elif DECISION_TREE == feature_selection_strategy:
                    estimator = DecisionTreeClassifier(random_state=random_values[0], class_weight="balanced")

                elif RANDOM_FOREST == feature_selection_strategy:
                    estimator = RandomForestClassifier(random_state=random_values[0], class_weight="balanced")
                ...
                if estimator is None:
                    if ANOVA_CHI2 == feature_selection_strategy:
                        ...
                        using_columns = input_defaulter_set_with_dummy_variables.columns[:-1].tolist()
                else:
                    rfecv = RFECV(estimator=estimator, step=1, cv=StratifiedKFold(5, random_state=((random_values[0] + 100) % const.RANDOM_RANGE[1]) + const.RANDOM_RANGE[0]), scoring=bcr_scorer, n_jobs=8)
                    rfecv.fit(input_set, output_set)

                    columns = input_defaulter_set_with_dummy_variables.iloc[:, :-1].columns
                    active_columns = rfecv.get_support()
                    using_columns = [columns[i] for i in range(len(active_columns)) if active_columns[i]]
                    print("Optimal number of features : %d" % rfecv.n_features_)
                    if grid_scores is None:
                        grid_scores = rfecv.grid_scores_
                    else:
                        grid_scores = [grid_scores[i] + rfecv.grid_scores_[i] for i in range(len(grid_scores))]
                ...
                print(input_defaulter_set_with_dummy_variables[using_columns + data_set["classification_label"]].columns)
                Parallel(n_jobs=cpu_count)(delayed(execute_classifier_run)(random_values, input_defaulter_set_with_dummy_variables[using_columns + data_set["classification_label"]],
